...
PRINT'-------CRIANDO UMA VIEW PARA O TOTAL DE VENDAS POR PRODUTO-------';
...
PRINT'-------CRIANDO UMA VIEW PARA TOP 3 CLIENTES QUE MAIS COMPRARAM -------';
CREATE VIEW vw_top3_clientes AS
	SELECT TOP 3 c.nome_cliente, SUM(v.quantidade) AS 'total vendido'
	FROM clientes c
	JOIN vendas v ON c.cliente_id=v.cliente_id
	GROUP BY c.nome_cliente;
...
SELECT * FROM vw_top3_clientes;

PRINT'-------CRIANDO UMA VIEW PARA O VALOR MEDIO POR CLIENTE-------';
CREATE VIEW vw_valor_medio AS
	SELECT c.nome_cliente, AVG(v.valor_total) AS 'media', COUNT(v.venda_id) as 'total'
	FROM clientes c
	JOIN VENDAS V ON c.cliente_id=v.cliente_id
	GROUP BY c.nome_cliente;
...
SELECT * FROM vw_valor_medio;


PRINT'-------CRIANDO UMA VIEW PARA O TOP 3 CLIENTES EM QUANTIDADE-------';
